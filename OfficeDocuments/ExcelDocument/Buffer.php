...
class App_Util_OfficeDocuments_ExcelDocument_Buffer
{
    protected $fd=null;
    protected $buffer='';
    ...
    public function __construct($filename, $fd_fopen_flags='w', $check_utf8=false)
    {
        ...
        if ($this->fd===false) {
            throw new App_Util_OfficeDocuments_Exception("Unable to open $filename for writing.");
        }
    }
    ...
    protected function purge()
    {
        if ($this->fd) {
            if ($this->check_utf8 && !self::isValidUTF8($this->buffer)) {
                ...
                throw new App_Util_OfficeDocuments_Exception("Error, invalid UTF8 encoding detected.");
            }
